...
#check if JSON content to avoid send it to Orthanc
def IsJson(bytes):
  try:
    contentString = bytes.decode('utf-8')
    json_object = json.loads(contentString)
...
def UploadFile(path):
    ...
    if IsJson(content):
       total_json_count+= 1
       sys.stdout.write("Importing %s => json file skiped \n" % path)
    else: 
        try:
...
if success_count == total_file_count:
    ...
    print("\nSummary: %d out of %d non JSON files have been imported successfully as DICOM instances" % (success_count, (total_file_count - total_json_count) ))
